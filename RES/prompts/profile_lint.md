...
Rewrite to exactly 2 blocks (ASCII only):
  ...
  Line 2 (Summary paragraph): One paragraph, max 45 words, 2 sentences preferred. Their problem, your pattern, how you solve. No metrics.
...
- **No metrics in Summary paragraph:** no digits, no percentages, no $ amounts, no x/× multipliers, no time durations. Do not insert PROOF_SNIPPET. If the current profile contains any number or metric pattern, strip it and rewrite qualitatively.
...
---USER---
Profile angle (ground truth — use THEIR_PROBLEM, ANALOGOUS_EXPERIENCE, SOLUTION_PATTERN only; PROOF_SNIPPET is for Summary method bullets / Experience, not the Summary paragraph):
{profile_angle}
