...
- Data: may reuse details/metrics from `RES/assets/archive_pdfs/` and portfolio HTMLs; tailor for competitive SF/tech market.
- State: store run history in repo (Markdown file).

## source_files_for_resume_cover_letter
- Core prompts: `RES/prompts/*.md` (experience, skills statements, mission statement). Legacy human workflow: `docs/RES/legacy_prompt.md`.
...
- Keep application run history in-repo Markdown (e.g., `RES/data/history.md`) with inputs/outputs/notes per run.
