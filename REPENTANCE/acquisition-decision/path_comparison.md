...
```mermaid
flowchart LR
  subgraph likely [Most likely 12m outcome]
    A[Path A: stalled without CTO fix]
    B[Path B: offer at ops-AI company]
    C[Path C: still searching or bad fit close]
  end
  subgraph best [Best case 12m]
    A2[Path A: 5-10 logos ARR ramp]
    B2[Path B: hired + shipping with eng team]
    C2[Path C: closed product-led SMB]
  end
```
